	...
	while (runGame) { //loop forever
		...
		if(gameState == GAME_LOAD){
			...
			*/
			...
			playerParticles = InitParticleSystem(&emitter, *particleSprite);
			...
			playerStartPos.x = 87;
			playerStartPos.y = 46;
			playerObject = InitPlayer(playerSprite, playerStartPos, 16, 16);
			...
			//Advance Game State as the level has finished loading
			gameState = GAME_RUN;
